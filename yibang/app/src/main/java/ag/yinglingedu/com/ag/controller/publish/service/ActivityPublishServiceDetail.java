...
import ag.yinglingedu.com.ag.bean.request.ReqSentService;
import ag.yinglingedu.com.ag.bean.response.BaseResponseBean;
...
public class ActivityPublishServiceDetail extends BaseActivity {
        ...
        gvShow.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position == mList.size() - 1) {
                    Intent intent = new Intent(ActivityPublishServiceDetail.this, PhotoSelectorActivity.class);
...
//                    if (paths.size() < 6 && mList.size() != 1){
    ...
    private void getData() {
        ReqSentService netBean = new  ReqSentService(Utils.getSpUtils().getString(Config.USER_ID, ""),Utils.getSpUtils().getString(Config.TOKEN, ""),);
        ...
        HttpUtil.post(netBean, new RequsetUtils.OnCompleteListener() {
            ...
            public void success(String result, int line) {
                BaseResponseBean responseBean = JSON.parseObject(result, BaseResponseBean.class);
                if (responseBean.getResult() > 0) {
                    ToastUtils.showShortToast(responseBean.getRetmsg());
                }
            }
